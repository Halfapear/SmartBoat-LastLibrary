...
{ 2, 5, 6, 10, 10, 10, 10, 10, 10, 10,
  10, 11, 12, 13, 14, 14, 15, 16, 17, 17,
  18, 18, 19, 20, 21, 22,24,25,27,28,
...
void Get_Four_jiao()//调试用，获得4个角点
{
    LUJ=Find_Left_Up_Point(80, 20);
    RUJ=Find_Right_Up_Point(80, 20);
    LDJ=Find_Left_Down_Point(115, 40);
    RDJ=Find_Right_Down_Point(115, 40);
}
//获取左右边界的丢线次数
void GetLostTime()
{
...
int Ring_Start_Test()
{
    if(rd.Left_Lost_Time>rd.Right_Lost_Time&&
        ...
        Monotonicity_Change_Left(90,30)!=0&&Monotonicity_Change_Right(90,30)==0&&
...
void Left_Ring()
{
    ...
    if(rd.state==1)
            {
                monotonicity_change_line[0]=Monotonicity_Change_Left(50,10);//寻找单调性改变点
    ...
    else if(rd.state==3)//3状态准备进环，寻找上拐点，连线
            {
                if(k!=0)
                {
                    if(k<1.3)
                        k=1.3;
                    K_Draw_Line(k,MT9V03X_W-5,MT9V03X_H-10,0);//k是刚刚算出来的，静态变量存着
                ...
                else
                {
                    ...
                    if(k==0&&(15<=Left_Up_Guai[0]&&Left_Up_Guai[0]<60)&&(50<Left_Up_Guai[1]&&Left_Up_Guai[1]<130)&&Left_Up_Guai[0]>5)//拐点出现在一定范围内，认为是拐点出现
                    {
                        ...
                        K_Draw_Line(k,MT9V03X_W-30,MT9V03X_H-10,0);//记录下第一次上点出现时位置，针对这个环岛拉一条死线，入环
                        image_process();//刷新边界数据
                    }
                }
    ...
    else if(rd.state==4)//状态4已经在里面
     {
        if(abs(FJ_Angle)>240)//积分200度以后在打开出环判断
        {
            ...
            {
             if(k==0&&k_flag==0){
                     monotonicity_change_line[0]=Monotonicity_Change_Right(115,30);//单调性改变
                     monotonicity_change_line[1]=l_border[monotonicity_change_line[0]];
                     k=(float)((float)(MT9V03X_H-monotonicity_change_line[0])/(float)(island_state_5_down[1]-monotonicity_change_line[1]));
                     k_flag=1;
             ...
             if(k!=0){
                ...
                if((rd.state==5)&&(abs(FJ_Angle)>342 ))//右边先丢线
                {
                    rd.state=7;
                }
...
//            if(rd.state==2&&(Boundry_Start_Right>=MT9V03X_H-10))//右下角再不丢线进3
    ...
    else if(rd.state==4)//环内状态，无特殊处理
    {
        if(abs(FJ_Angle)>240)//积分200度以后在打开出环判断
        {
            ...
            {
                island_state_5_down[0]=MT9V03X_H-2;
                island_state_5_down[1]=l_border[MT9V03X_H-2]-5;
                if(k==0)
                    k=(float)((float)(MT9V03X_H-monotonicity_change_line[0])/(float)(island_state_5_down[1]-monotonicity_change_line[1]));
    ...
    else if(rd.state==5)//准备出环岛
    {
        ...
        if(k!=0){
         ...
         if(rd.state==5&&(60>rd.L_Edgepoint_y))//左边先丢线
         {
               rd.state=6;
         }
    ...
    else if(rd.state==6)//继续出
         {
            ...
            k=-2;
             ...
             if(((rd.state==6)&&(rd.L_Edgepoint_y>MT9V03X_H-10))||(abs(FJ_Angle)>310))//左边先丢线-此处可以添加陀螺仪判据积分300度
             {//
                 k=0;
                 rd.state=7;
             }
...
//这里选择变11次才算（我记得好像没那么多）
        ...
        if (region > 2&&s>12&&rd.Ring_Flag==0&&Cross_Flag==0) {
            garageout_flag = 1;
...
void Cross_Detect()
{
    ...
    if(rd.state==0&&rd.Ring_Flag==0)//与环岛互斥开
    {
        Left_Up_Find=0;
        Right_Up_Find=0;
        if(rd.Both_Lost_Time>=20)//十字必定有双边丢线，在有双边丢线的情况下再开始找角点
        {
